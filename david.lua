...
local JsonCache = {} 
local ROOT_MOUNT_PATH = "UDIN PENGEN NAIK HAJI/mount_data/" 
...
Title = "UDIN PENGEN NAIK HAJI| Auto Walk",
    Icon = "navigation",
    NewElements = true,
    Theme = "Crimson", 
    User = {
...
local MOUNT_CONFIG_URL = "scripts/mount_config.lua"
...
Tabs.WalkTab:Toggle({
    Title = "Bypass AFK",
    Desc = "Mencegah kick AFK otomatis.", 
    Type = "Checkbox",
    Default = false,
    Callback = function(v)
        if v then
            if not antiAfkConnection then
                ...
                DimszyHUB16:Notify({ Title = "Anti AFK", Content = "Aktif.", Duration = 2 })
            end
        else
            if antiAfkConnection then antiAfkConnection:Disconnect() antiAfkConnection = nil DimszyHUB16:Notify({ Title = "Anti AFK", Content = "Nonaktif.", Duration = 2 }) end
        end
    end
})
...
Tabs.WalkTab:Toggle({
    Title = "Auto Loop",
    Desc = "Mengulang auto walk secara otomatis.",
    Default = false,
    Callback = function(v)
        isLoopingEnabled = v
        if not v then isLoopingActive = false end
        DimszyHUB16:Notify({ Title = "Loop", Content = v and "Loop Aktif." or "Loop Mati.", Duration = 2 })
    end
})
...
-- ANIMATION TAB
pcall(function()
local Section = Tabs.RunAnimationTab:Section({ Title = "Animation Pack List" })

local RunAnimations = {
    ["Run Animation 1"] = {
        Idle1="rbxassetid://1083445855", Idle2="rbxassetid://1083450166",
        Walk="http://www.roblox.com/asset/?id=656121766", Run="rbxassetid://656118852",
        Jump="rbxassetid://1083455352", Fall="http://www.roblox.com/asset/?id=1083443587",
        Climb="http://www.roblox.com/asset/?id=129447497744818", Swim="http://www.roblox.com/asset/?id=10921264784",
        SwimIdle="http://www.roblox.com/asset/?id=10921265698"
    },
    ["Run Animation 2"] = {
        Idle1="rbxassetid://1083445855", Idle2="rbxassetid://1083450166",
        Walk="http://www.roblox.com/asset/?id=18537392113", Run="rbxassetid://18537384940",
        Jump="rbxassetid://1083455352", Fall="http://www.roblox.com/asset/?id=1083443587",
        Climb="http://www.roblox.com/asset/?id=129447497744818", Swim="http://www.roblox.com/asset/?id=10921264784",
        SwimIdle="http://www.roblox.com/asset/?id=10921265698"
    },
...
local function SaveOriginal(Char)
    ...
    for _, child in ipairs(Animate:GetDescendants()) do
        if child:IsA("Animation") then originals[child] = child.AnimationId end
    end
...
local function ApplyAnimation(Char, animName)
    ...
    -- pastikan field name cocok (Idle1, Idle2, Walk, Run, Jump, Fall, Climb, Swim, SwimIdle)
    pcall(function()
        Animate.idle.Animation1.AnimationId = pack.Idle1 or Animate.idle.Animation1.AnimationId
        Animate.idle.Animation2.AnimationId = pack.Idle2 or Animate.idle.Animation2.AnimationId
        Animate.walk.WalkAnim.AnimationId = pack.Walk or Animate.walk.WalkAnim.AnimationId
        Animate.run.RunAnim.AnimationId = pack.Run or Animate.run.RunAnim.AnimationId
        Animate.jump.JumpAnim.AnimationId = pack.Jump or Animate.jump.JumpAnim.AnimationId
        Animate.fall.FallAnim.AnimationId = pack.Fall or Animate.fall.FallAnim.AnimationId
        Animate.climb.ClimbAnim.AnimationId = pack.Climb or Animate.climb.ClimbAnim.AnimationId
        Animate.swim.Swim.AnimationId = pack.Swim or Animate.swim.Swim.AnimationId
        Animate.swimidle.SwimIdle.AnimationId = pack.SwimIdle or Animate.swimidle.SwimIdle.AnimationId
        Humanoid.Jump = true
    end)
end
...
local function RestoreOriginal(Char)
    ...
    OriginalAnimations[Char] = nil
end

for i = 1, 2 do
    local name = "Run Animation " .. i
    Tabs.RunAnimationTab:Toggle({
        Title = "[◉] " .. name,
        CurrentValue = false,
        Callback = function(Value)
            local Char = Players.LocalPlayer.Character
            if not (Char and Char:FindFirstChild("Animate") and Char:FindFirstChild("Humanoid")) then
                DimssHUB:Notify({ Title = "Animation", Content = "Character/Animate/Humanoid tidak ditemukan.", Duration = 2 })
                return
            end
            if Value then
                SaveOriginal(Char)
                ApplyAnimation(Char, name)
                DimssHUB:Notify({ Image = "person-standing", Title = name, Content = "Berhasil diterapkan.", Duration = 3 })
            else
                RestoreOriginal(Char)
                DimssHUB:Notify({ Image = "person-standing", Title = name, Content = "Berhasil dimatikan.", Duration = 3 })
            end
        end,
    })
end
...
local ThemeToggle = Tabs.SettingsTab:Toggle({
    ...
    Callback = function(state)
        if canChangeTheme then
            local newTheme = state and "Dark" or "Crimson"
...
-- ===============================
-- FORCE KEY LOGIN SETIAP EXECUTE
-- ===============================
_G.KEY_UNLOCKED = false

-- OPTIONAL: matiin auto login total
local ENABLE_AUTO_LOGIN = false
...
KeyTab:Button({
    ...
    Callback = function()
        ...
        task.spawn(function()
            local ok, data = ValidateToken(typedToken)

            if ok then
                saveToken(typedToken)
                _G.KEY_UNLOCKED = true

                -- BUAT TAB SETELAH KEY VALID
                Tabs.MainTab = Window:Tab({
                    Icon = "globe",
                    Title = "MAIN SERVER",
                    Locked = false
                })
                Tabs.WalkTab = Window:Tab({
                    Title = "AUTO WALK",
                    Icon = "file",
                    Locked = false
                })
                Tabs.RunAnimationTab = Window:Tab({
                    Title = "ANIMATION",
                    Icon = "braces",
                    Locked = false
                })
                ...
                DimszyHUB16:Notify({
                    Title = "Success",
                    Content = "Key valid! Semua menu sudah aktif.",
                    Duration = 2
                })
            else
                DimszyHUB16:Notify({
...
-- ===============================
-- AUTO LOGIN (DIMATIKAN DEFAULT)
-- ===============================
if ENABLE_AUTO_LOGIN then
    task.spawn(function()
        task.wait(1)
        local saved = loadToken()
        if saved and saved ~= "" then
            local ok = ValidateToken(saved)
            if ok then
                _G.KEY_UNLOCKED = true
            end
        end
...
-- ===============================
-- AUTO WALK SAFETY (PUNYA LU)
-- ===============================
local function setupAutoWalkSafety(character)
    local humanoid = character:FindFirstChildOfClass("Humanoid")
    ...
    deathConnection = humanoid.Died:Connect(function()
        if isPlaying then
            ...
            DimszyHUB16:Notify({
                Title = "Auto Walk",
                Content = "Dihentikan otomatis: Karakter jatuh/mati.",
                Duration = 3
            })

            if currentMountType == "CHECKPOINT" then
                for _, otherTg in ipairs(activeUIElements) do
                    if otherTg.Type == "Toggle"
                        and string.sub(otherTg.Title, 1, 1) ~= "S"
                        and otherTg.CurrentValue then
                        pcall(function()
                            otherTg:SetValue(false)
                        end)
                    end
...
local deathConnection = setupAutoWalkSafety(character)
...
player.CharacterAdded:Connect(function(newChar)
    ...
    deathConnection = setupAutoWalkSafety(newChar)

    if isPlaying then
        stopPlayback(true)
        if FloatingUI then FloatingUI.Hide_All() end
        if SimpleFloatingControls then SimpleFloatingControls.Hide() end

        DimszyHUB16:Notify({
            Title = "Auto Walk",
            Content = "Karakter respawn — Auto Walk dihentikan.",
            Duration = 3
        })
    end
end)
